...
def get_onion_list():
    ...
    if os.path.exists("missing_kids_onions.txt"):

        with open("missing_kids_onions.txt") as fd:
...
def store_onion(onion):
    ...
    with codecs.open("missing_kids_onions.txt", "ab", encoding = "utf8") as fd:
        fd.write("%s\n" %onion)
...
def run_onionscan(onion):
    ...
    process = subprocess.Popen(["onionscan", "-jsonReport", 
...
print("[INFO] Starting onionscan")

onion = 'msydqstlz2kzerdg.onion'
...
print("[INFO] count is {}".format(count))  

            
#except Exception as e:
#print("Sorry Got a {}".format(e))
sys.exit(1)
